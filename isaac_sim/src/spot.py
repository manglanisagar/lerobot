...
# Load prop configuration
with open("/root/Documents/props_config.json", "r") as f:
    props_config = json.load(f)

# Spawn each prop based on config
for idx, prop in enumerate(props_config):
    prop_path = f"/World/Prop_{idx}"
    define_prim(prop_path, "Xform")
    
    full_usd_path = assets_root_path + prop["path"]
    prop_prim = get_prim_at_path(prop_path)
    prop_prim.GetReferences().AddReference(full_usd_path)

    prop_xform = UsdGeom.Xformable(stage.GetPrimAtPath(prop_path))
    prop_xform.AddTranslateOp().Set(tuple(prop["translation"]))
    prop_xform.AddRotateXYZOp().Set(tuple(prop["rotation"]))

    # Make the object physical and collidable
    RigidPrim(prop_path)
    GeometryPrim(prop_path).apply_collision_apis()
